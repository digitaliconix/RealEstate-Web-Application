        ...
        <b-card :title="`${title} Unit`">
            <b-row>
                ...
                <b-col md="6">
                    <b-form-group
                        label="Project"
                        label-for="mc-project-id"
                    >
                        <x-tag-input resource="/api/search/projects"
                                     :value="form.project" @input="onProjectUpdate"
                                     column="title" name="title"
                        >
                        </x-tag-input>
                        <error :error="error.project_id"/>
                    </b-form-group>
                </b-col>
                <b-col md="6">
                    <b-form-group
                    ...
                    >
                        <x-tag-input :resource="`/api/search/buildings?project_id=${form.project_id}`"
                                     ...
                                     :disabled="form.project_id == null ? true : false"
                                     column="title" name="title"
                        >
                        </x-tag-input>
            ...
            <template v-if="form.charges.length > 0">
                ...
                <b-row v-for="(item,index) in form.charges" :key="index">
                    <b-col md="5">
                        <b-form-group
                            label="Charge Type"
                            label-for="mc-charge-type-id"
                        >
                            <x-tag-input :resource="`/api/search/project_charge_types?project_id=${form.project_id}`"
                                         :value="item.project_charge_type"
                                         @input="onProjectChargeTypeUpdate($event, index)"
                                         :disabled="form.project_id == null ? true : false"
                                         column="title" name="title"
                            >
                            </x-tag-input>
                            <error :error="error[`charges.${index}.project_charge_type_id`]"/>
                        </b-form-group>
                    </b-col>
                    <b-col md="5">
                        <b-form-group
                        ...
                        >
                            <b-form-input
                                ...
                                v-model="item.price"
                            />
                            <error :error="error[`charges.${index}.price`]"/>
                        </b-form-group>
                    </b-col>
                    <b-col
                        md="2"
...
<script>
    ...
    function initialize(to) {
        let urls = {
            'add': `/api/units/create`,
            'edit': `/api/units/${to.params.id}/edit`
        };
    ...
    export default {
        ...
        methods: {
            ...
            onProjectChargeTypeUpdate(e, index) {
                for(var i=0;i<this.form.charges.length;i++){
                    if(this.form.charges[i].project_charge_type_id == e.id){
                        this.$toast({
                ...
                Vue.set(this.$data.form.charges[index], 'project_charge_type', e);
                Vue.set(this.$data.form.charges[index], 'project_charge_type_id', e.id);
            },
            addCharges() {
